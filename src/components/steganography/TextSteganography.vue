  ...
  <div class="text-steganography">
    ...
    <v-window v-model="activeTab">
      <v-window-item value="hide">
        ...
        <v-card v-if="resultText" class="mb-8 pa-4" outlined>
                ...
                <v-expansion-panel title="Šifrování heslem (volitelné)">
                  <v-expansion-panel-text>
                    ...
                    <v-alert v-if="encryptionType !== 'none'" type="warning" variant="tonal" density="comfortable" class="mt-2">
                      <strong>Pozor:</strong> Pokud zapomenete heslo, nebude možné zprávu obnovit!
                    </v-alert>
                  </v-expansion-panel-text>
                </v-expansion-panel>
              </v-expansion-panels>
      ...
      <v-window-item value="reveal">
        <v-card class="mb-8 pa-4" outlined>
          ...
          <v-card-text>
            ...
            <v-expand-transition>
              <div v-if="(selectedRevealMethod.value === 'delimiters' || selectedRevealMethod.value === 'base64-delimiters') && selectedRevealMethod.supportsCustomDelimiters">
    ...
    <v-dialog v-model="showFileNameDialog" max-width="500px">
      <v-card>
        <v-card-title>Stáhnout jako TXT soubor</v-card-title>
        <v-card-text>
          <v-text-field v-model="downloadFileName" label="Název souboru" outlined suffix=".txt" autofocus variant="outlined"></v-text-field>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="primary" text @click="showFileNameDialog = false">Zrušit</v-btn>
          <v-btn color="primary" @click="downloadTextFile">Stáhnout</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
function performHideInText() {
  ...
  try {
    ...
    setTimeout(() => {
      const resultCard = document.querySelector('.text-steganography .v-card + .v-card');
      if (resultCard) {
...
function performRevealFromText() {
  ...
  try {
    ...
    setTimeout(() => {
      const resultCard = document.querySelector('.result-alert');
      if (resultCard) {
        resultCard.scrollIntoView({ behavior: 'smooth', block: 'start' });
      }
    }, 100);
...
function copyToClipboard() {
  ...
  navigator.clipboard.writeText(textToCopy)
    .then(() => {
      emit('show-message', {
        message: encryptionType.value !== 'none' 
...
watch(() => selectedMethod.value?.value, (newMethod) => {
  if (newMethod) {
...
watch(() => selectedRevealMethod.value?.value, (newMethod) => {
  if (newMethod) {
